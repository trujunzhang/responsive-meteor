...
class AppAdminHistoryList extends Component {
    ...
    constructor(props) {
        ...
        this.onHistoryActionEventClick = this.onHistoryActionEventClick.bind(this);
    ...
    renderTableHeader() {
        return (
          ...
          <tr>
              ...
              <th scope="col" id="author" className="manage-column column-curator">Scraped Post</th>
    ...
    onHistoryActionEventClick(type, cb) {
    ...
    actionEvent(type, itemIds, item, cb) {
        switch (type) {
            ...
            case "quick_edit":
                ...
                break;
            case "preview":
                this.context.messages.pushRouter(this.props.router, {pathname: "/", query: {historyId: item._id, title: item.slug}});
                break;
            case "trash":
                this.context.actions.call('history.move.to.trash', itemIds, (error, result) => {
                    cb(error, result);
                });
                break;
            case "restore":
                this.context.actions.call('history.restore.to.last.status', itemIds, (error, result) => {
                    cb(error, result);
                });
                break;
            case "delete_permanently":
                ...
                if (window.confirm(deleteFolderConfirm)) {
                    this.context.actions.call('history.delete.permanently', itemIds, (error, result) => {
                        cb(error, result);
                    });
                }
                break;
              ...
              <tr>
                  <td>
                      <div className="row" id="table_no_items">
                          <div className="admin-table-loading">No Scraped History found.</div>
                      </div>
                  </td>
              </tr>
              </tbody>
...
AppAdminHistoryList.contextTypes = {
...
AppAdminHistoryList.displayName = "AppAdminHistoryList";

module.exports = withRouter(AppAdminHistoryList);
export default withRouter(AppAdminHistoryList);

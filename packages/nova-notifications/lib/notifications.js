...
Telescope.notifications.create = (userIds, notificationName, data) => {
    ...
    userIds.forEach(userId => {
        ...
        let userEmail = Users.getEmail(user);
        if (!user && !userEmail && notificationName === "removedUser") {
            userEmail = Users.getEmail(data);
        }
        if (!!userEmail) {
            NovaEmail.buildAndSendHTML(userEmail, subject, html);
        } else {

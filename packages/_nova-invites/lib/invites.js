...
Telescope.callbacks.add("users.new.sync", setStartingInvites);
...
function checkIfInvited (user) {
    ...
    let invite = Invites.findOne({ invitedUserEmail : Users.getEmail(user) });

    if(invite){

        let invitedBy = Meteor.users.findOne({ _id : invite.invitingUserId });
...
Telescope.callbacks.add("users.profileCompleted.async", checkIfInvited);

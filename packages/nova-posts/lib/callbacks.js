...
function PostsNewRateLimit(post, user) {
    ...
    if (!Users.isAdmin(user)) {
        ...
        let timeSinceLastPost = Users.timeSinceLast(user, Posts),
...
function PostsNewRequiredPropertiesCheck(post, user) {
    ...
    const defaultProperties = {
        createdAt: new Date(),
        postedAt: new Date(),// also add "postedAt" without be approved,
...
//        let postAuthor = Meteor.users.findOne(post.userId);
...
function PostsNewNotifications(post) {
    ...
    if (typeof Telescope.notifications !== "undefined") {
        ...
        let notifiedUserIds = _.pluck(Users.find({'telescope.notifications_posts': true}, {fields: {_id: 1}}).fetch(), '_id');
...
function PostsNewStasticTopics(post, topicsArray) {
    const topics = topicsArray;
    if (!!topics) {
        Posts.statisticTopics(topics);
    }
}
Telescope.callbacks.add("posts.new.async", PostsNewStasticTopics);
...
//function PostsEditRunPostApprovedCallbacks(post, oldPost, topicsArray, currentUser) {
 ...
 */
function PostsEditStasticTopics(modifier, post, topicsArray) {
    const topics = topicsArray;
    if (!!topics) {
        Posts.statisticTopics(topics);
    }
}
Telescope.callbacks.add("posts.edit.async", PostsEditStasticTopics);
...
function postsApprovedNotification(post) {
    if (typeof Telescope.notifications !== "undefined") {
        ...
        if (!!user && !user.isAdmin) {
            Telescope.notifications.create(post.userId, 'postApproved', {post: post});
        }
    }
}
Telescope.callbacks.add("posts.approve.async", postsApprovedNotification);

function postsRejectedNotification(flag, user) {
    if (typeof Telescope.notifications !== "undefined") {
        if (!!user) {
            const post = Posts.findOne(flag.postId);
            Telescope.notifications.create(post.userId, 'postRejected', {post: post});
        }
    }
}
Telescope.callbacks.add("flags.new.async", postsRejectedNotification);

function postsRemovedNotification(posts) {
    if (typeof Telescope.notifications !== "undefined") {

        _.forEach(posts, function (post) {
            const user = Users.findOne(post.userId);
            if (!!user && !user.isAdmin) {
                Telescope.notifications.create(post.userId, 'postRemoved', {post: post});
            }
        });
    }
}
Telescope.callbacks.add("posts.delete.permanently.async", postsRemovedNotification);

// ------------------------------------- users.remove.async -------------------------------- //

//function UsersRemoveDeletePosts (user, options) {
//    if (options && options.deletePosts) {
//        var deletedPosts = Posts.remove({userId: user._id});
//    } else {

...
/*
 ...
 - UsersRemoveDeleteCommentsAndPosts
...
function CommentsNewRequiredPropertiesCheck(comment, user) {
    ...
    let userId = comment.userId; // at this stage, a userId is expected
...
//
//        let commentAuthor = Users.findOne(comment.userId);
//
//        // upvote comment
        ...
        if (Meteor.isServer && !comment.disableNotifications) {
            ...
            if (!!comment.parentCommentId) {
                ...
                if (parentComment.userId !== post.userId && parentComment.userId !== comment.userId) {
                    ...
                    let parentCommentAuthor = Users.findOne(parentComment.userId);

                    // do not notify parent comment author if they have reply notifications turned off
                    // TODO: DJZHANG (9/01/2016)
                    // parentCommentAuthor, this user maybe has already been deleted.
                    // So before notification, need to check whether is null.
                    if (!!parentCommentAuthor && Users.getSetting(parentCommentAuthor, "notifications_replies", true)) {
...
function UsersRemoveDeleteCommentsAndPosts(user, options) {
    const userId = user._id;

    // upvoters
    Comments.find({upvoters: userId}).fetch().forEach(function (comment) {
    ...
    // downvoters
    Comments.find({downvoters: userId}).fetch().forEach(function (comment) {
        Comments.update(comment._id, {
            $inc: {downvotes: -1},
    ...
    // upvoters
    Posts.find({upvoters: userId}).fetch().forEach(function (post) {
    ...
    // downvoters
    Posts.find({downvoters: userId}).fetch().forEach(function (post) {
        Posts.update(post._id, {
            $inc: {downvotes: -1},
            $pull: {downvoters: userId}
        });
    ...
    //    // 1. findAllCommentsByUserId.
    //    Comments.find({userId: userId}).fetch().forEach(function (comment) {
    //        // 3. update post
    //        Posts.update(comment.postId, {
    //            $inc: {commentCount: -1},
    ...
    if (options && options.deletePosts) {
        //Posts.find({userId:user._id}).fetch().
        const deletedPosts = [];
        Posts.find({userId: user._id}).forEach(function (post) {
...
Telescope.callbacks.add("users.remove.async", UsersRemoveDeleteCommentsAndPosts);

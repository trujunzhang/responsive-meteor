...
Meteor.methods({
  ...
  'newsletter.addUser'(user){
    ...
    if (!user || !Users.canEdit(currentUser, user)) {
      ...
      try {
          const result = MailChimpList.add(user, false);
          Telescope.callbacks.runAsync("newsletter.addUser.async", user);
          return result;
    } catch (error) {
  ...
  'newsletter.removeUser'(user) {
    ...
    if (!user || !Users.canEdit(currentUser, user)) {
      ...
      try {
          const result =  MailChimpList.remove(user);
          Telescope.callbacks.runAsync("newsletter.removeUser.async", user);
          return result;
    } catch (error) {
      throw new Meteor.Error(500, error.message);
    }

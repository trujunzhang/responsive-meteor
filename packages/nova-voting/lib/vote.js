...
Telescope.operateOnItem = function (collection, itemId, user, operation) {
  ...
  let item = collection.findOne(itemId);
  let votePower = Telescope.getVotePower(user);
  let hasUpvotedItem = user.hasUpvoted(item);
  let hasDownvotedItem = user.hasDownvoted(item);
  let update = {};
  ...
  const collectionName = item.getCollectionName();
  ...
  let result = collection.update({_id: item._id}, update);
  ...
  if (result > 0) {
    ...
    //  if(item.getCollectionName() == "posts") {// TODO: djzhang callbacks for posts and comments now.
          Telescope.callbacks.runAsync(collectionName + "." + operation + ".async", item, user, collection, operation);
      //}
